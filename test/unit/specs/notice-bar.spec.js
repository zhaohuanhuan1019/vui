...
describe('NoticeBar', () => {
  ...
  it('create a notice-bar', () => {
    ...
    expect(wrapper.find('.vu-notice-bar').length).to.equal(1);
  });
  ...
  it('mode closeable', () => {
    ...
    const icon = wrapper.find('.vu-icon-close');
  ...
  it('mode link', () => {
    ...
    expect(wrapper.find('.vu-icon-arrow').length).to.equal(1);
  });
  ...
  it('notice-bar transitionend', (done) => {
    ...
    const content = wrapper.find('.vu-notice-bar__content')[0];
    setTimeout(() => {
