import Picker from 'packages/picker/src/picker.vue';
import PickerColumn from 'packages/picker/src/pickercolumn.vue';
...
describe('Picker', () => {
  ...
  it('create picker', () => {
    ...
    expect(wrapper.hasClass('vu-picker')).to.be.true;
  ...
  it('create a empty picker and emit a cencel event', (done) => {
    ...
    expect(wrapper.hasClass('vu-picker')).to.be.true;
    expect(wrapper.contains('.vu-picker__toolbar')).to.be.true;
    expect(wrapper.vm.currentColumns.length).to.equal(0);

    const eventStub = sinon.stub(wrapper.vm, '$emit');
    const cancelBtn = wrapper.find('.vu-picker__cancel')[0];
  ...
  it('create a empty picker and emit a confirm event', (done) => {
    ...
    const eventStub = sinon.stub(wrapper.vm, '$emit');
    const cancelBtn = wrapper.find('.vu-picker__confirm')[0];
    cancelBtn.trigger('click');
...
describe('PickerColumn', () => {
  ...
  it('create a picker-column', () => {
    ...
    expect(wrapper.hasClass('vu-picker-column')).to.be.true;
  ...
  it('disabled options', () => {
    ...
    expect(wrapper.find('.vu-picker-column--disabled').length).to.equal(1);
  ...
  it('drag options', () => {
    ...
    const column = wrapper.find('.vu-picker-column')[0];
    dragHelper(column, 0);
    expect(wrapper.vm.currentIndex).to.equal(0);
