  ...
  methods: {
    ...
    onTouchMove (event) {
      ...
      if (this.direction === 'horizontal') {
        ...
        if (this.endFunction && ((this.active === 0 && this.deltaX > 0) || (this.active === this.count - 1 && this.deltaX < 0))) {
          this.offset = (this.deltaX > 0 ? 2 : -2) * Math.sqrt(Math.abs(this.deltaX)) - (this.active + 1) * this.width;
        } else {
    ...
    onTouchEnd () {
      if (this.deltaX) {
        if (this.endFunction && ((this.active === 0 && this.deltaX > 0) || (this.active === this.count - 1 && this.deltaX < 0))) {
          if (this.deltaX <= -1 * this.endOffset) {
            this.endFunction();
          }
          this.currentDuration = this.duration;
          this.offset = -(this.active + 1) * this.width;
        } else {
